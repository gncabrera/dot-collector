...
import com.dot.collector.api.service.EnvironmentVariableService;
...
/**
 * REST controller for managing {@link com.dot.collector.api.domain.EnvironmentVariable}.
 */
@RestController
@RequestMapping("/api/environment-variables")
public class EnvironmentVariableResource {

    private static final Logger LOG = LoggerFactory.getLogger(EnvironmentVariableResource.class);
    ...
    public ResponseEntity<EnvironmentVariableDTO> createEnvironmentVariable(
        ...
        environmentVariableDTO = environmentVariableService.save(environmentVariableDTO);
        return ResponseEntity.created(new URI("/api/environment-variables/" + environmentVariableDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, environmentVariableDTO.getId().toString()))
            .body(environmentVariableDTO);
    }
    ...
    public ResponseEntity<EnvironmentVariableDTO> updateEnvironmentVariable(
        ...
        environmentVariableDTO = environmentVariableService.update(environmentVariableDTO);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, environmentVariableDTO.getId().toString()))
            .body(environmentVariableDTO);
    }
    ...
    public ResponseEntity<EnvironmentVariableDTO> partialUpdateEnvironmentVariable(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, environmentVariableDTO.getId().toString())
        );
    }

    /**
     * {@code GET  /environment-variables} : get all the Environment Variables.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of Environment Variables in body.
     */
    @GetMapping("")
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<EnvironmentVariableDTO> getEnvironmentVariable(@PathVariable("id") Long id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteEnvironmentVariable(@PathVariable("id") Long id) {
