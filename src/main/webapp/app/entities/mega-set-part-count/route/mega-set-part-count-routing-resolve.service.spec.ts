...
import megaSetPartCountResolve from './mega-set-part-count-routing-resolve.service';

describe('MegaSetPartCount routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IMegaSetPartCount returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          megaSetPartCountResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(megaSetPartCountResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
