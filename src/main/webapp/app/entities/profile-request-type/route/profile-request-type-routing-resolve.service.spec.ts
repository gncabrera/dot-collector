...
import profileRequestTypeResolve from './profile-request-type-routing-resolve.service';

describe('ProfileRequestType routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IProfileRequestType returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          profileRequestTypeResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(profileRequestTypeResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
