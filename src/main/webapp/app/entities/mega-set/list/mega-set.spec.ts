...
import { sampleWithRequiredData } from '../mega-set.test-samples';
import { MegaSetService } from '../service/mega-set.service';

import { MegaSet } from './mega-set';

vitest.useFakeTimers();

describe('MegaSet Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: MegaSet;
  let fixture: ComponentFixture<MegaSet>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(MegaSet);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 26766 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.megaSets()[0]).toEqual(expect.objectContaining({ id: 26766 }));
  });

  describe('trackId', () => {
    it('should forward to megaSetService', () => {
      const entity = { id: 26766 };
      vitest.spyOn(service, 'getMegaSetIdentifier');
      const id = comp.trackId(entity);
      expect(service.getMegaSetIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.megaSetsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
