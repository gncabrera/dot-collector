...
import megaAttributeOptionResolve from './mega-attribute-option-routing-resolve.service';

describe('MegaAttributeOption routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IMegaAttributeOption returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          megaAttributeOptionResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(megaAttributeOptionResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
