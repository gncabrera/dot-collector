...
import { sampleWithRequiredData } from '../profile-collection.test-samples';
...
describe('ProfileCollection Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ProfileCollection;
  let fixture: ComponentFixture<ProfileCollection>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ProfileCollection);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 29855 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.profileCollections()[0]).toEqual(expect.objectContaining({ id: 29855 }));
  });

  describe('trackId', () => {
    it('should forward to profileCollectionService', () => {
      const entity = { id: 29855 };
      vitest.spyOn(service, 'getProfileCollectionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getProfileCollectionIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.profileCollectionsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
