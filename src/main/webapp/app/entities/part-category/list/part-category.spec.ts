...
import { sampleWithRequiredData } from '../part-category.test-samples';
...
describe('PartCategory Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: PartCategory;
  let fixture: ComponentFixture<PartCategory>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PartCategory);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 10447 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.partCategories()[0]).toEqual(expect.objectContaining({ id: 10447 }));
  });

  describe('trackId', () => {
    it('should forward to partCategoryService', () => {
      const entity = { id: 10447 };
      vitest.spyOn(service, 'getPartCategoryIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPartCategoryIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.partCategoriesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
